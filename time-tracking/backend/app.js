...
const { mongoUri, sessionSecret } = require("./config/server-config");
...
app.use(
  cors({ origin: ["https://iu-time-tracking.click"], credentials: true })
);
...
app.get("/", (req, res, next) => {
  try {
    res.status(200).json({ message: "Request received from API.4" });
  } catch {
  ...
  if (req.body.actId) {
    Activity.deleteOne({ _id: req.body.actId }).then((result) => {
      try {
        ...
        res.status(500).json({
          message: "Internal server error deleting item (logged in).",
        });
      }
    });
    return;
